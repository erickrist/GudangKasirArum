...
// ==========================================
// 1. EXPORT PDF MASTER (LAPORAN LENGKAP)
// ==========================================
export const exportMasterPDF = ({
  ...
  const dateStr = `${startDate || 'Awal'} s/d ${endDate || 'Sekarang'}`;

  // HEADER
  doc.setFontSize(16); doc.setTextColor(13, 148, 136); // Teal color
  doc.text("LAPORAN KEUANGAN ARZEN FROZEN FOOD", 14, 15);
  doc.setFontSize(10); doc.setTextColor(100);
  doc.text(`Cabang: ${storeName} | Periode: ${dateStr}`, 14, 22);
  ...
  if (inList.length > 0) {
    ...
    doc.text("1. PEMASUKAN PENJUALAN (Uang Laci Masuk)", 14, currentY); currentY += 5;
    let totalIn = 0;
    const inBody = inList.map(t => {
      ...
      return [
        ...
        t.note || (t.items ? t.items.map(i => i.name).join(', ') : 'Transaksi Pemasukan'), 
  ...
  // --- TABEL 2A: BIAYA OPERASIONAL ---
  if (opsExpenses.length > 0) {
    checkPageBreak(30);
    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.setTextColor(0);
    doc.text("2A. BIAYA OPERASIONAL (Gaji, Listrik, Retur, dll)", 14, currentY); currentY += 5;
    let totalOps = 0;
  ...
  // --- TABEL 2B: BIAYA KULAKAN ---
  if (kulakanExpenses.length > 0) {
    checkPageBreak(30);
    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.setTextColor(0);
    doc.text("2B. BIAYA KULAKAN / RESTOCK BARANG", 14, currentY); currentY += 5;
    let totalKulakan = 0;
  ...
  // --- TABEL 3: HISTORI HUTANG ---
  if (filteredDebtHistory && filteredDebtHistory.length > 0) {
    checkPageBreak(30);
    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.setTextColor(0);
    doc.text("3. HISTORI HUTANG", 14, currentY); currentY += 5;
  ...
  // --- TABEL 5: HISTORI DEPOSIT ---
  if (filteredDepositHistory && filteredDepositHistory.length > 0) {
    checkPageBreak(30);
    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.setTextColor(0);
    doc.text("5. HISTORI DEPOSIT", 14, currentY); currentY += 5;
  ...
  // --- TABEL 7: SALDO BERSIH ---
  if (filteredNetBalance && filteredNetBalance.length > 0) {
    checkPageBreak(30);
    doc.setFontSize(11); doc.setFont("helvetica", "bold"); doc.setTextColor(0);
    doc.text("7. SALDO BERSIH ALIRAN KAS LACI", 14, currentY); currentY += 5;
  ...
  onShowToast('File PDF Lengkap berhasil diunduh', 'success');
};

// ==========================================
// 2. EXPORT PDF NERACA SALDO
// ==========================================
...
// ==========================================
// 3. EXPORT PDF LABA RUGI (SINKRON DASHBOARD & KULAKAN)
// ==========================================
export const exportLabaRugiPDF = ({ totalIncome, totalHPP, totalPureOperational, totalKulakan = 0, totalDamagedGoods, startDate, endDate, storeName, onShowToast }) => {
  ...
  const totalBiaya = totalPureOperational + totalDamagedGoods;
  ...
  const body = [
    ...
    ['Total Omset Penjualan (Barang Keluar)', `Rp ${totalIncome.toLocaleString('id-ID')}`],
    ['Total Modal Barang Keluar (HPP Terkunci)', `- Rp ${totalHPP.toLocaleString('id-ID')}`],
    [{ content: 'LABA KOTOR', styles: { fontStyle: 'bold' } }, `Rp ${labaKotor.toLocaleString('id-ID')}`],
    ['', ''],
    [{ content: 'BEBAN & KERUGIAN', colSpan: 2, styles: { fontStyle: 'bold', fillColor: [240, 240, 240] } }],
    ['Beban Operasional Laci', `- Rp ${totalPureOperational.toLocaleString('id-ID')}`],
    ['Kerugian Retur / Barang Rusak', `- Rp ${totalDamagedGoods.toLocaleString('id-ID')}`],
    ['', ''],
    [{ content: 'LABA BERSIH AKHIR (NET)', styles: { fontStyle: 'bold', fontSize: 12 } }, { content: `Rp ${labaBersih.toLocaleString('id-ID')}`, styles: { fontStyle: 'bold', fontSize: 12, textColor: labaBersih >= 0 ? [0, 128, 0] : [220, 38, 38] } }],
    ['', ''],
    [{ content: 'INFO ARUS KAS LAINNYA', colSpan: 2, styles: { fontStyle: 'bold', fillColor: [255, 243, 205] } }],
    ['Belanja Kulakan / Restock (Aset Gudang)', `Rp ${totalKulakan.toLocaleString('id-ID')}`],
  ];
  ...
  doc.save(`LabaRugi_${storeName}_${Date.now()}.pdf`);
  onShowToast('File PDF Laba Rugi berhasil diunduh', 'success');
};

// ==========================================
// 4. EXPORT PDF LABA PER BARANG (DENGAN RINCIAN UTUH + ECERAN)
// ==========================================
export const exportKeuntunganPDF = (profitData, startDate, endDate, storeName, onShowToast) => {
  if (profitData.length === 0) return onShowToast('Tidak ada data transaksi di periode ini', 'error');

  const doc = new jsPDF('l', 'mm', 'a4'); // Mode Landscape
  doc.setFontSize(14); doc.setFont("helvetica", "bold"); 
  doc.text("LAPORAN LABA / KEUNTUNGAN PER BARANG", 14, 15);
  
  doc.setFontSize(10); doc.setFont("helvetica", "normal");
  doc.text(`Toko/Cabang: ${storeName} | Periode: ${startDate || 'Awal'} - ${endDate || 'Sekarang'}`, 14, 22);

  let totalM = 0, totalP = 0, totalU = 0;

  const body = profitData.map(item => {
    totalM += item.totalHpp;
    totalP += item.netSales;
    totalU += item.profit;

    return [
      item.name,
      item.displaySold || '-',       // SUDAH FORMAT "1 BALL + 5 PCS"
      item.displayReturned || '-',   // SUDAH FORMAT "1 BALL + 5 PCS"
  ...
  body.push([
    ...
    { content: `Rp ${totalM.toLocaleString('id-ID')}`, styles: { fontStyle: 'bold' } },
    { content: `Rp ${totalP.toLocaleString('id-ID')}`, styles: { fontStyle: 'bold' } },
    { content: `Rp ${totalU.toLocaleString('id-ID')}`, styles: { fontStyle: 'bold', textColor: totalU < 0 ? [220, 38, 38] : [0, 128, 0] } }
  ]);

  autoTable(doc, {
    startY: 30,
    head: [['Nama Barang', 'Terjual (Utuh + Ecer)', 'Diretur (Utuh + Ecer)', 'Modal (HPP Terkunci)', 'Omset Jual', 'Laba Bersih']],
    body: body,
    styles: { fontSize: 9 },
  ...
  doc.save(`Laba_Barang_${storeName}_${Date.now()}.pdf`);
  onShowToast('Laporan Laba Per Barang PDF berhasil diunduh', 'success');
};

// ==========================================
// 5. EXPORT PDF HISTORI STOK
// ==========================================
export const exportHistoristockPDF = (filteredHistory, startDate, endDate, storeName, formatDisplayDate, onShowToast) => {
  ...
  doc.setFontSize(14); doc.setFont("helvetica", "bold"); 
  doc.text(`Laporan Histori Pergerakan Stok Barang`, 14, 15);
  
  doc.setFontSize(10); doc.setFont("helvetica", "normal"); 
  doc.text(`Cabang Filter: ${storeName} | Periode: ${startDate || 'Semua Data'} s/d ${endDate || 'Sekarang'}`, 14, 21);
  ...
  autoTable(doc, { 
    ...
    body, startY: 30, styles: { fontSize: 9 },
    didParseCell: function(data) {
  ...
  onShowToast('Laporan Histori Stok PDF berhasil diunduh', 'success');
};